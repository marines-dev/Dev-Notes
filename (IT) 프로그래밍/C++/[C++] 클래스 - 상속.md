...
상속은 기존 클래스(부모 클래스)를 다른 클래스(자식 클래스)가 물려받아 부모 클래스의 public과 protected의 멤버(변수, 메서드 등)에 접근이 가능하여 클래스 간의 계층적인 구조를 갖게된다.

이를 통해 코드의 중복을 줄이고, 프로그램을 더 효율적으로 구성할 수 있다. 

- 부모 클래스 : 다른 클래스에게 상속을 제공한다.
- 자식 클래스 : 기본 클래스를 상속받아 기능을 확장하거나 수정할 수 있다.
...
> **상속 관계에서 생성자 호출 시점**
> 
> 자식 클래스의 객체 생성 시 부모 클래스의 생성자가 호출된 후 자식클래스의 생성자가 호출된 다음 객체가 생성된다. 
